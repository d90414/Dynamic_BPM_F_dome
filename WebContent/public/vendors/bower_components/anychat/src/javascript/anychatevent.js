...
function OnAnyChatTransBuffer(dwUserId, lpBuf, dwLen) {
	/*var result = lpBuf.split("|");
	if (result[0] == "5100") {
		$("#VideoDiv").hide();
		if (result[1] == "0") { // 视频认证通过
			$("#success").click();
		} else { // 视频认证未通过
			$("#failed").click();
			DisplayTextMessage(dwUserId, ("系统消息：" + result[2])
					.fontcolor("#800000"));
		}
	}*/
}
...
function OnAnyChatRecordSnapShot(dwUserId, lpFileName, dwParam, bRecordType) {
	lpFileName = lpFileName.replace(/\\/g, "\/");
	AddLog("OnAnyChatRecordSnapShot return");
	AddLog("dwUserId=(" + dwUserId + ")");
	AddLog("lpFileName=(" + lpFileName + ")");
	AddLog("dwParam=(" + dwParam + ")");
	AddLog("bRecordType=(" + bRecordType + ")");

	/*var taskid = BRAC_TransFile(0, lpFileName, 0, 0, 0);
	AddLog("taskid=(" + taskid + ")");*/
	var szUrl = BRAC_GetSDKOptionStringEx(BRAC_SO_LOCALPATH2URL, lpFileName, 0);
	AddLog("OnAnyChatRecordSnapShot" + dwUserId + "|" + lpFileName + "|" + dwParam + "|" + bRecordType + "|" + szUrl);
	// 关闭本地视频
	...
	if (bRecordType == 0) {
		...
		if (szUrl.substring(szUrl.length - 4) == ".mp4") {
			RECOED_FILE_PATH = lpFileName;
			RECOED_FILE_URL = szUrl;
		}
	}
}
...
function OnAnyChatConnect(bSuccess, errorcode) {
	AddLog("OnAnyChatConnect(errorcode=" + errorcode + ")");
	if (errorcode == 0) {
...
function OnAnyChatLoginSystem(dwUserId, errorcode) {
	AddLog("OnAnyChatLoginSystem(userid=" + dwUserId + ", errorcode=" + errorcode + ")");
	if (errorcode == 0) {
		ConfigAnyChatParameter();
		mSelfUserId = dwUserId;
		/*if (mSeatID != null) {
			errorcode = BRAC_VideoCallControl(BRAC_VIDEOCALL_EVENT_REQUEST,
					mSeatID, 0, 0, 0, "");
			AddLog("发起视频请求,errorcode:" + errorcode);
			BRAC_TransBuffer(mSeatID, "5200|10100");
		}*/
	} else {
...
function OnAnyChatEnterRoom(dwRoomId, errorcode) {
	AddLog("OnAnyChatEnterRoom(roomid=" + dwRoomId + ", errorcode=" + errorcode + ")");
	if (errorcode == 0) {
		ApplyVideoConfig(0, 3, 417, 278, 15, 3);
		// 打开本地视频
		var result = BRAC_UserCameraControl(-1, 1);
		AddLog("打开本地视频，返回码：" + result);
		// 打开本地语音
		result = BRAC_UserSpeakControl(-1, 1);
		AddLog("打开本地音频，返回码：" + result);
		// 设置本地视频显示位置
		showVideoSessionScreen();
	} else {
...
function OnAnyChatRoomOnlineUser(dwUserCount, dwRoomId) {
	/*AddLog("OnAnyChatRoomOnlineUser(usercount=" + dwUserCount + ", roomid=" + dwRoomId + ")");
	if (mSeatID != null) {
		// 请求对方音频、视频
		var result1 = BRAC_UserCameraControl(mSeatID, 1);
		AddLog("打开远程视频，返回码：" + result1 + "mSeatID:" + mSeatID);
		result1 = BRAC_UserSpeakControl(mSeatID, 1);
		AddLog("打开远程音频，返回码：" + result1 + "mSeatID:" + mSeatID);
		// 设置视频显示位置
		result1 = BRAC_SetVideoPos(mSeatID, document.getElementById("AnyChatRemoteVideoDiv"),
		"ANYCHAT_VIDEO_REMOTE");
		AddLog("设置位置，返回码：" + result1);
	}*/
}

// 用户进入（离开）房间，dwUserId表示用户ID号，bEnterRoom表示该用户是进入（1）或离开（0）房间
function OnAnyChatUserAtRoom(dwUserId, bEnterRoom) {
	/*AddLog("OnAnyChatUserAtRoom(userid=" + dwUserId + ", benter=" + bEnterRoom + "mTargetUserId=" + mTargetUserId + ")");
	if (dwUserId == mSeatID && bEnterRoom != 0) {
		// 请求对方音频、视频
		var errorcode = BRAC_UserCameraControl(mSeatID, 1);
		errorcode = BRAC_UserSpeakControl(mSeatID, 1);
		result1 = BRAC_SetVideoPos(mSeatID, document
				.getElementById("AnyChatRemoteVideoDiv"), "ANYCHAT_VIDEO_REMOTE");
		AddLog("设置位置，返回码：" + result1);
	}*/
}
...
function OnAnyChatLinkClose(reason, errorcode) {
	var msg = "跟视频服务器连接断开，原因是" + reason + "，错误代码是" + errorcode + "正在尝试重连，请检测网络";
	AddLog(msg);
}
...
function OnAnyChatVideoCallEvent(dwEventType, dwUserId, dwErrorCode, dwFlags, dwParam, szUserStr) {
	...
	AddLog("OnAnyChatVideoCallEvent(dwEventType=" + dwEventType + ", dwUserId=" + dwUserId + ", dwErrorCode=" + dwErrorCode + ", dwFlags=" + dwFlags + ", dwParam=" + dwParam + ", szUserStr=" + szUserStr + ")");*/
}
